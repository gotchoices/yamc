...
char *s;
 ...
 while(len)
  ...
  if(x)
   ...
   else if(((s[1]>='a' && s[1]<='z') || (s[1]>='A' && s[1]<='Z')) && srch->pieces[(s[1]&0x1f)-1])
    {
...
int *notify;
 ...
 for(x=0;s[x];++x)
  ...
  case 'k': case 'K': srch->restrct=1; break;
...
SRCH *srch;
 {
 if(!srch->markb || !srch->markk || !srch->restrct) return 0;
