  ...
  int flg;
{
  ...
  if(l<w->top->line+w->h && (l+n>=w->top->line+w->h || (l+n==w->b->eof->line && w->b->eof->line>=w->top->line+w->h))) {
    if(l>=w->top->line) /* Update window from l to end */
      msetI(w->t->t->updtab+w->y+l-w->top->line,1,w->h-(int)(l-w->top->line));
    else				  /* Update entire window */
      msetI(w->t->t->updtab+w->y,1,w->h);
  } else if(l<w->top->line+w->h && l+n==w->b->eof->line && w->b->eof->line<w->top->line+w->h) {
    if(l>=w->top->line) /* Update window from l to end of file */
      msetI(w->t->t->updtab+w->y+l-w->top->line,1,(int)n);
    else				  /* Update from beginning of window to end of file */
...
if(amnt) do
 ...
 if(square)
  if(bc=='\t')
   ...
   if(tcol>from && tcol<=to) c=INVERSE;
   else c=0;
   }
  else
...
if(amnt) do
 ...
 if(square)
  if(bc=='\t')
   ...
   if(scr+x-ox>=from && scr+x-ox<to) c=INVERSE;
   else c=0;
 else
  if(byte>=from && byte<to) c=INVERSE;
  else c=0;
...
if(markv(0) && markk->b==w->b) {
  ...
  if(marking && markb && markb->b==w->b && w->cursor->byte!= markb->byte && !from) {
    if(square) {
      ...
      from=Lmin(w->cursor->byte,markb->byte),
  ...
  int prompt;
{
  ...
  if(window==window->main)
   {
   rmkbd(window->kbd);
   window->kbd=mkkbd(getcntext(w->o.context));
   } 
