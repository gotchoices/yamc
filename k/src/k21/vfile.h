...
#define ILIMIT (PGSIZE*1024)	/* Max amount to buffer */
#define HTSIZE 1024		/* Entries in hash table.  Must be pwr of 2 */
#define INC 64			/* Pages to allocate each time */
 ...
 *
 * Also:  This function does not allocate space to the file.  Use jvalloc()
 ...
 * ever might want to is to implement your own version of jvalloc()).
 */
...
#define wc(v,a,c) \
	( \
	  (v)->addr==((a)&~(PGSIZE-1)) ? \
	   ...
	   ((a)+1>vsize(v) && jvalloc(v,(a)+1-vsize(v))), \
