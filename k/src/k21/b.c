...
unsigned char *s;
 ...
 if(!len) return p;
 p->valcol=0;
 mset(table,0,256); for(x=0;x!=len-1;++x) table[toup(s[x])]=x;
 ffwrd(p,len); amnt-=len; x=len; do
  if((c=toup(frgetc(p)))!=toup(s[--x]))
   {
   if(x<=table[c]) ffwrd(p,len-x+1), --amnt;
   else ffwrd(p,len-table[c]), amnt-=x-table[c];
   if(amnt<0) return 0;
   else x=len;
   }
...
unsigned char *s;
 ...
 if(!len) return p;
 mset(table,0,256); for(x=len;--x;table[toup(s[x])]=x);
 x=0; do
  if((c=toup(fpgetc(p)))!=toup(s[x++]))
   {
   if(len-table[c]<=x) fbkwd(p,x+1), --amnt;
   else fbkwd(p,len-table[c]), amnt-=len-table[c]-x;
   if(amnt<0) return 0;
   else x=0;
   }
